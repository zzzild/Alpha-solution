...
import {
  ...
  DIFFICULTY_COLOR,
  METHOD_COLOR,
} from "./PelatihanConstants";
import { inputCls, formatPrice } from "./PelatihanUtils";

/**
 * Field — wrapper label + input + pesan error untuk satu field form.
 * Didefinisikan di sini karena hanya dipakai oleh PelatihanModal.
 *
 * @param {string}      label    - Teks label
 * @param {boolean}     required - Tampilkan tanda bintang merah jika true
 * @param {string}      error    - Pesan error; ditampilkan di bawah input
 * @param {JSX.Element} children - Elemen input/select/textarea
 */
...
/**
 * PelatihanModal — dialog multi-mode untuk operasi CRUD paket.
 *
 * Mode yang didukung:
 ...
 *
 * @param {Object}   props
 * @param {Object}   props.modal    - { mode, item? }
 * @param {Function} props.onClose  - Tutup modal
 * @param {Function} props.onCreate - Simpan paket baru
 * @param {Function} props.onUpdate - Simpan perubahan paket
 * @param {Function} props.onDelete - Hapus paket berdasarkan id
 */
export default function PelatihanModal({
  ...
  // Inisialisasi form: kosong saat create, salin data item saat mode lain
  ...
  /** Update satu field dan reset error-nya sekaligus. */
  const set = (key, val) => {
    setForm((f) => ({ ...f, [key]: val }));
    setErrors((e) => ({ ...e, [key]: "" }));
  };

  /** Validasi field wajib; return objek error (kosong = semua valid). */
  const validate = () => {
    const e = {};
          ...
          {mode === "view" && (
            <div className="space-y-5">
              ...
              <div className="flex items-start gap-4">
                ...
                <p className="text-xl font-bold text-blue-600 shrink-0">
  ...
  />
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 shrink-0">
          ...
          {/* Konfirmasi hapus — hanya mode delete */}
          {mode === "delete" && (
            <button
          ...
          {/* Simpan — hanya mode create dan edit */}
          {(mode === "create" || mode === "edit") && (
            <button
