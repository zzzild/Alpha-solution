...
const addPaket = async (req, res) => {
    try{
        const { namePaket, description, price, duration, jumlahModul, tingkatKesulitan, masaAkses, sertifikat, metode } = req.body;
        const imageFile = req.file;

        if (!namePaket || !description || !price || !duration || !jumlahModul || !tingkatKesulitan || !masaAkses || !sertifikat || !metode ) {
            return res.json({ success: false, message: "All fields are required" });
        }
...
const updatePaket = async (req, res) => {
    try {
            ...
            if (!namePaket || !description || !price || !duration || !jumlahModul || !tingkatKesulitan || !masaAkses || !sertifikat || !metode || !duration) {
                return res.json({ success: false, message: "All fields are required" });
            }
            ...
            const updatedPaket = await paketModel.findOneAndUpdate({paketId}, updateData, { new: true });
            if (!updatedPaket) {
                return res.json({ success: false, message: "Paket not found" });
            }

            return res.json({ success: true, message: "Paket updated successfully", paket: updatedPaket });
...
const deleteKriteria = async (req, res) => {
    try {
        ...
        const deletedKriteria = await kriteriaModel.findOneAndDelete({kriteriaId});
        if (!deletedKriteria) {
...
const updateKriteria = async (req, res) => {
    try {
        ...
        const updatedKriteria = await kriteriaModel.findOneAndUpdate({kriteriaId}, updateData, { new: true });

        if (!updatedKriteria) {
            return res.json({ success: false, message: "Kriteria not found" });
        }

        res.json({ success: true, message: "Kriteria updated successfully", kriteria: updatedKriteria });
    } catch (error) {
        console.error(error);
...
export { loginAdmin, registerAdmin, addPaket, deletePaket, updatePaket, addKriteria, deleteKriteria, updateKriteria, getPemesanan};